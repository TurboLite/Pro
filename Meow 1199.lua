...
local Window = Fluent:CreateWindow({
    Title = "Turbo Lite Hub | Pro Premium",
...
local Section = Tabs.Main:AddSection("Main Setup Setting")
...
local LinhTinh = Tabs.an:AddSection("Setting Main")

local Toggle = Tabs.an:AddToggle("Bypass TP", { Title = "Stop TP Bypass When Have Legendary", Default = false })
Toggle:OnChanged(function(Value)
    _G.StopTP = Value
end)
...
local Toggle = Tabs.an:AddToggle("Turn V4", { Title = "Auto Active Race V4", Default = false })
Toggle:OnChanged(function(Value)
    RaceNguLon = Value
end)
